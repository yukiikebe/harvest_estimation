...
from create_doy_prediction_input.seeding_estimator import SeedingEstimate
...
def _write_crop_summary_csv(
    ...
    if seeding_estimate is not None and seeding_estimate.greenup_start is not None and seeding_estimate.greenup_start not in value_map:
        all_ts.append(seeding_estimate.greenup_start)
    if seeding_estimate is not None and seeding_estimate.seeding_date is not None and seeding_estimate.seeding_date not in value_map:
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for ts in all_ts:
            ...
            if (np.isnan(v_ndvi) and np.isnan(v_ndwi) and np.isnan(v_evi)) and not (is_start or is_end or is_greenup or is_seeding):
                continue
            ...
            if seeding_event:
                greenup_rule = seeding_estimate.rule if seeding_estimate is not None else ""
                greenup_source = seeding_estimate.source_index if seeding_estimate is not None else ""
                seeding_offset_days = "" if seeding_estimate is None or seeding_estimate.offset_days is None else str(seeding_estimate.offset_days)
                seeding_window_start = seeding_estimate.window_start if seeding_estimate is not None else ""
                seeding_window_end = seeding_estimate.window_end if seeding_estimate is not None else ""
            else:
                greenup_rule = ""
                greenup_source = ""
                seeding_offset_days = ""
